...
export function RoundResultBoard({ entries, myPlayerId, roundIndex, totalRounds }: Props) {
  return (
    <div className="screen result-screen">
      ...
      <span className="eyebrow">ROUND {String(roundIndex + 1).padStart(2, '0')} / {String(totalRounds).padStart(2, '0')}</span>
      <h2>이번 라운드 결과</h2>
      ...
      <ul className="result-ranking">
        {entries.map((entry, index) => (
          ...
          >
            ...
            <span className="rank-score">+{entry.roundScore.toLocaleString()}</span>
